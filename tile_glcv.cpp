...
void initGL()
{
    ...
    //視点極座標初期値
    eDist = 5000; eDegX = 0.0; eDegY = 0.0;
}
...
void display()
{
    ...
    //theta = grayImage.at<unsigned char>(grayImage.rows/2, grayImage.cols/2);
    //cv::Vec3d s = frameImage.at<cv::Vec3d>(frameImage.rows/2, frameImage.cols/2);
    //r = s[2];
    //g = s[1];
    //b = s[0];
    ...
    for(int j=0; j<24; j++){
        for(int i=0; i<32; i++){
            theta = grayImage.at<unsigned char>((j*grayImage.rows/24), (i*grayImage.cols/32));
            if(theta>128){
                glEnable(GL_LIGHT0);  //光源0
            }else{
                glDisable(GL_LIGHT0);  //光源0
            ...
            glPushMatrix();  //行列一時保存
            glTranslated(-1550.0+i*100.0, 1150.0-j*100, 0.0);  //中心座標
            glRotated(theta, 1.0, 0.0, 0.0); //回転(角度、回転軸)
            glScaled(95.0, 95.0, 20.0);  //拡大縮小
    ...
    glMaterialf(GL_FRONT, GL_SHININESS, 64);  //ハイライト係数
    glPushMatrix();  //行列一時保存
    glTranslated(-200.0, -100.0, 0.0);  //中心座標
    glScaled(100.0, r+1.0, 30.0);  //拡大縮小
    glutSolidCube(1.0);  //立方体の配置
    glPopMatrix();  //行列復帰
    ...
    glMaterialf(GL_FRONT, GL_SHININESS, 64);  //ハイライト係数
    glPushMatrix();  //行列一時保存
    glTranslated(0.0, -100.0, 0.0);  //中心座標
    glRotated(0, 1.0, 0.0, 0.0); //回転(角度、回転軸)
    glScaled(100.0, g+1.0, 30.0);  //拡大縮小
    glutSolidCube(1.0);  //立方体の配置
    glPopMatrix();  //行列復帰
    ...
    glTranslated(200.0, -100.0, 0.0);  //中心座標
    glRotated(0, 1.0, 0.0, 0.0); //回転(角度、回転軸)
    glScaled(100.0, b+1.0, 30.0);  //拡大縮小
    glutSolidCube(1.0);  //立方体の配置
    glPopMatrix();  //行列復帰
    */
